...
*   **热键系统**:
    *   **启用方式**: 热键功能默认不启用，需通过 `options.enableHotkeys: true` 显式开启。
    *   **多实例支持**: 每个组件实例拥有独立的热键管理，解绑函数保存在组件作用域中，多个 `RoiEditor` 实例可同时使用而不产生冲突。
    *   **选择工具**: `V`
    *   **框选工具**: `M`
    *   **撤销**: `Ctrl+Z`
    *   **重做**: `Ctrl+Y`
...
*   **热键启用开关**: 工具默认不注册热键事件，可通过 `options.enableHotkeys: true` 显式启用键盘热键功能。
